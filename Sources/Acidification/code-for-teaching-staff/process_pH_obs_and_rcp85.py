...
filename_obs='../../../Data-for-teaching-staff/Acidification/pHT_median_historical.nc';
filename_rcp85='../../../Data-for-teaching-staff/Acidification/pHT_median_ssp585.nc';
...
pH_obs = np.asarray(ncfile_obs.variables['pHT'][:]);
pH_rcp85 = np.asarray(ncfile_rcp85.variables['pHT'][:]);
pH_obs_years=np.asarray(ncfile_obs.variables['time'][:]);
pH_rcp85_years=np.asarray(ncfile_rcp85.variables['time'][:]);
...
pH_obs_global_decadal_mean=np.nansum(pH_obs*delta_area*ocean_mask,axis=(1,2))/total_area
pH_rcp85_global_decadal_mean=np.nansum(pH_rcp85*delta_area*ocean_mask,axis=(1,2))/total_area
...
# plot globally averaged pH time series:
plt.figure(1,figsize=(15,5))
plt.plot(pH_obs_years,pH_obs_global_decadal_mean)
plt.plot(pH_rcp85_years,pH_rcp85_global_decadal_mean)
plt.xlabel("Year")
plt.ylabel("pH")
plt.grid(lw=0.25)
plt.show()

# contour pH at 1850,2010 and 2100:
pH_obs_1850_map=pH_obs[pH_obs_years==1850,:,:][0,:,:]
pH_obs_2010_map=pH_obs[pH_obs_years==pH_obs_years[-1],:,:][0,:,:]
pH_rcp85_2100_map=pH_rcp85[pH_rcp85_years==2100,:,:][0,:,:]

plt.figure(1,figsize=(5,8))

plt.subplot(3,1,1)
c=plt.contourf(longitude, latitude, pH_obs_1850_map[:,:],20)
plt.xlabel('Longitude')
plt.ylabel('Latitude')
clb=plt.colorbar(c, shrink=0.85, pad=0.02)
clb.set_label('pH')
plt.title('pH, rcp85, year=1850')

plt.subplot(3,1,2)
c=plt.contourf(longitude, latitude, pH_obs_2010_map[:,:],20)
plt.xlabel('Longitude')
plt.ylabel('Latitude')
clb=plt.colorbar(c, shrink=0.85, pad=0.02)
clb.set_label('pH')
plt.title('pH, obs, year='+repr(int(pH_obs_years[-1].item())))

plt.subplot(3,1,3)
c=plt.contourf(longitude, latitude, pH_rcp85_2100_map[:,:],20)
plt.xlabel('Longitude')
plt.ylabel('Latitude')
clb=plt.colorbar(c, shrink=0.85, pad=0.02)
clb.set_label('pH')
plt.title('pH, rcp85, year=2100')
...
np.save("Output/to-pickle/pH_obs_1850_map.npy",pH_obs_1850_map)
np.save("Output/to-pickle/pH_obs_2010_map.npy",pH_obs_2010_map)
np.save("Output/to-pickle/pH_rcp85_2100_map.npy",pH_rcp85_2100_map)
...
print("saved to Output/to-pickle/")
