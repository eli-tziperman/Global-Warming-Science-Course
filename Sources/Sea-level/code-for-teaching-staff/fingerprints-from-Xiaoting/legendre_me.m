function y = legendre_me(n,x,normalize)
...
%   $Revision: 5.22.4.7 $  $Date: 2010/08/23 23:13:11 $
%
%   Reference:
...
if nargin < 2
    error(message('MATLAB:legendre:TooFewInputs'))
end
...
elseif strcmp(normalize,'me')
    % Calculate the fully-normalized functions.
    % For m = 0,...,n, multiply by (-1)^m*sqrt(n+1/2)
    % For m = 0,...,n, multiply by sqrt(2pi)*sqrt(n+1/2)
    % y = sqrt((2*n+1) / (4*pi))*y;
    % y = sqrt((n+1/2) * pi/2)*y;
    y = sqrt(2*n+1)*y;    
    const1 = -1;
